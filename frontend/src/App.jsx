        ...
        <div className="w-full lg:w-2/3">
          {loading ? (
            <div className="h-full min-h-[500px] flex flex-col items-center justify-center border border-slate-800 rounded-2xl bg-slate-900/50 backdrop-blur-sm">
              ...
              <p className="text-sm text-slate-500 mt-2">Running AI models and generating Terraform templates</p>
            </div>
          ) : error ? (
            ...
            <div className="flex flex-col h-full bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
              ...
              <div className="flex border-b border-slate-800 bg-slate-950/50 px-2 pt-2">
                {['diagram', 'explanation', 'cost', 'terraform'].map((tab) => (
                  <button
                    key={tab}
                  ...
                  <div>
                    <h3 className="text-xl font-semibold mb-4 text-slate-100 flex items-center justify-between">
                      ...
                      <span className="text-2xl font-bold text-emerald-400">
                        ${result.total_monthly_cost.amount.toFixed(2)}
                      </span>
                    </h3>
                    <div className="bg-slate-950 rounded-xl p-1 overflow-x-auto">
                        <div dangerouslySetInnerHTML={{ __html: result.cost_estimation_table.replace(/\\n/g, '<br/>') }} className="markdown-table-wrapper" />
                        {/* We will render markdown strings natively in the final iteration, for now basic formatting */}
                        <pre className="text-sm text-slate-300 p-4 font-mono">{result.cost_estimation_table}</pre>
                    </div>
                  </div>
                )}
                {activeTab === 'terraform' && (
            ...
            <div className="h-full min-h-[500px] flex flex-col items-center justify-center border border-dashed border-slate-800 rounded-2xl bg-slate-900/20 text-center p-8">
              ...
              <p className="text-slate-500 max-w-md">
                Fill out your application requirements on the left, and our AI will generate a complete cloud architecture, cost estimate, and deployment code.
              </p>
            </div>
          )}
        </div>
