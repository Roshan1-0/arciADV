...
api_key = os.environ.get("OPEsk-abcdef1234567890abcdef1234567890abcdef12NAI_API_KEY")
...
def get_mock_ai_response(req: ArchitectureRequest):
    ...
    ha_note = (
        f"The architecture is configured for {req.uptime}% uptime SLA with "
        f"{'multi-AZ deployment and read replicas' if very_high_uptime else 'high availability' if is_ha else 'single-zone deployment'}."
    )
    explanation = (
        ...
        f"{'Async processing via queue and serverless workers handles background tasks. ' if is_ml or high_traffic else ''}"
    ...
    components = [
        ...
        {"type": "compute",      "service_name": compute, "instance_size": compute_size, "count": compute_count, "notes": f"{compute_count} instance(s)"},
    ]
    if needs_db:
        components.append({"type": "database", "service_name": db_name,   "instance_size": db_size,   "count": 1, "notes": f"{req.db_type} database"})
    if needs_storage:
        components.append({"type": "storage",  "service_name": storage,   "instance_size": "Standard","count": 1, "notes": "Object storage"})
    if high_traffic or very_high_uptime:
        components.append({"type": "cache",    "service_name": db_cache,  "instance_size": "basic",   "count": 1, "notes": "In-memory cache"})
    if not is_api and (high_traffic or is_mobile) and needs_storage:
        components.append({"type": "cdn",      "service_name": cdn,       "instance_size": "medium",  "count": 1, "notes": "Global CDN"})
    if is_ml or (high_traffic and not is_mobile):
