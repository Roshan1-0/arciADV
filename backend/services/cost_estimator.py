...
def estimate_costs(cloud_provider: str, components: list, uptime: float = 99.0, runtime_months: int = 1) -> dict:
    ...
    aws_pricing = {
        ...
        "storage":     {"all": {"service": "S3 Standard",                "price": 2.0}},
        "loadbalancer":{"all": {"service": "Application Load Balancer",  "price": 2000.0}},
        "network":     {"all": {"service": "Route 53 Hosted Zone",       "price": 200.0}},
        "cdn": {
            "medium": {"service": "CloudFront Basic",    "price": 500.0},
            "high":   {"service": "CloudFront Advanced", "price": 1500.0},
        },
        "cache":      {"all": {"service": "ElastiCache Redis",         "price": 1200.0}},
        "serverless": {"all": {"service": "Lambda (per 1M req)",       "price": 0.20}},
    }
    ...
    azure_pricing = {
        ...
        "loadbalancer":{"all": {"service": "Standard Load Balancer",    "price": 1800.0}},
        "network":     {"all": {"service": "Azure DNS Hosted Zone",     "price": 150.0}},
        "cdn": {
            "medium": {"service": "Azure CDN Basic",    "price": 400.0},
            "high":   {"service": "Azure CDN Advanced", "price": 1200.0},
        },
        "cache":      {"all": {"service": "Azure Redis Cache",          "price": 1100.0}},
        "serverless": {"all": {"service": "Azure Functions (per 1M req)","price": 0.18}},
    }

    pricing = {"AWS": aws_pricing, "Azure": azure_pricing}.get(cloud_provider, aws_pricing)
    ...
    markdown_table = (
        "| Component | Service | Count | Unit Cost (INR/mo) | Subtotal (INR/mo) |\n"
        "| :--- | :--- | :---: | ---: | ---: |\n"
    )
    ...
    for comp in components:
        ...
        markdown_table += (
            ...
            f"INR {unit_cost:,.2f} | INR {line_total:,.2f} |\n"
        )
